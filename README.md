...
- Explains package.json, npm package-lock, and pnpm-lock dependency additions, removals, and version updates instead of only saying "lockfile changed."
...
- package.json, npm package-lock, and pnpm-lock dependency additions, removals, and updates.
...
PatchDrill summarizes dependency changes from changed `package.json`, npm `package-lock.json`, and `pnpm-lock.yaml` files, listing the package, dependency section or lockfile path, change type, previous version, and new version in Markdown and JSON reports. This complements heavier SCA tools by making reviewer-visible dependency intent explicit.
...
- Dependency diff enrichment for Yarn, Bun, Cargo, Go, and Python.
